...
# Genuine "nothing left in this bucket" — the only form that should block
# re-asking an attribute. Requires "additional"; the boundary scenario's
# one-time refusal below has the identical "I don't have a preference for X"
# shape without it, and must NOT be treated as exhaustion (the attribute is
# fully re-askable next turn once that one-time flag is spent).
NO_PREFERENCE_EXHAUSTED_RE = re.compile(
    r"\b(?:no|don't have|do not have)\s+(?:(?:an?|any)\s+)?additional\s+preference\b",
    re.I,
)
# Boundary's one-time "use your judgment" refusal — a no-op, not exhaustion.
NO_PREFERENCE_JUDGMENT_RE = re.compile(
    r"use your (?:judgment|discretion)|your call|up to you|"
    r"either (?:is|way) (?:fine|works|ok)|doesn't matter to me|not particular",
    re.I,
)
...
def _constraint_clauses(message: str, category: str) -> list[str]:
    if NO_PREFERENCE_EXHAUSTED_RE.search(message) or NO_PREFERENCE_JUDGMENT_RE.search(message):
        return []
    remainder = message
    if category:
...
class SessionState:
    ...
    def update(self, user_message: str) -> dict:
        ...
        if NO_PREFERENCE_EXHAUSTED_RE.search(message):
            if self.last_asked:
                self.declined.add(self.last_asked)
        elif NO_PREFERENCE_JUDGMENT_RE.search(message):
            pass
        else:
