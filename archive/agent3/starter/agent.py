...
# Mirrors starter/clarify.py's v1 policy from the sibling single-file agent: the
# evaluator's customer_reply() discloses up to 2 undisclosed intent-card items when
# asked "other", vs. at most 1 for a specific attribute that happens to match its
# classify_constraint() bucket. Asking "other" every turn front-loads disclosure
# faster; shopping.conversation's classify_constraint() slots the answer correctly
# regardless of which attribute prompted it, so this composes with the existing
# retrieval/ranking pipeline unchanged.
_OTHER_PROMPTS = [
...
class Agent:
    ...
    def _choose_other_first(self, state: SessionState, turn: int) -> tuple[str | None, str]:
        """Ask 'other' every turn until the intent card is genuinely exhausted.

        Boundary's one-time refusal ("I don't have a preference for X; please use
        your judgment") does not add to state.declined, so it doesn't stop this
        loop early. Genuine exhaustion ("I don't have an additional preference
        for other") does, via the NO_PREFERENCE_EXHAUSTED_RE branch in SessionState.update.
        """
        if turn >= 10 or "other" in state.declined:
            index = min(max(0, len(state.asked) - 1), len(_EXHAUSTED_MESSAGES) - 1)
            return None, _EXHAUSTED_MESSAGES[index]
        state.commit_question("other")
        return "other", _OTHER_PROMPTS[turn % len(_OTHER_PROMPTS)]
